...
fun main() {
    ...
    class Mover(
        ...
        fun getNext(): List<Mover> {
            return listOf(i - 1 to j, i + 1 to j, i to j - 1, i to j + 1)
                .filter { it.first in 0 until n && it.second in 0 until m && !visited[it.first][it.second][breakableCount] && (breakableCount > 0 || !map[it.first][it.second]) }
                .map {
                    visited[it.first][it.second][breakableCount] = true

                    if (breakableCount > 0 && map[it.first][it.second]) {
                        Mover(it.first, it.second, breakableCount - 1, times + 1)
                    } else {
                        Mover(it.first, it.second, breakableCount, times + 1)
                    }
                }
        }
    }
